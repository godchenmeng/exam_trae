...
namespace ExamSystem.Test
{
    ...
    public class MapDataConversionTest
    {
        ...
        {
            Console.WriteLine("=== 地图绘制数据转换功能测试 - 新格式 ===\n");
        ...
        static void TestNewFormatDataParsing()
        {
            Console.WriteLine("测试6: 新格式数据解析");
            ...
            {
                // 第一步：解析外层JSON
                var outerJson = JsonDocument.Parse(rawMapData);
                var dataProperty = outerJson.RootElement.GetProperty("data");
                var innerJsonString = dataProperty.GetString();
                
                Console.WriteLine($"  外层JSON解析成功");
                Console.WriteLine($"  内层JSON字符串: {innerJsonString?.Substring(0, Math.Min(100, innerJsonString?.Length ?? 0))}...");
                
                // 第二步：解析内层JSON
                var innerJson = JsonDocument.Parse(innerJsonString!);
                var mapData = innerJson.RootElement;
                
                // 提取基本信息
                if (mapData.TryGetProperty("center", out var centerElement))
                {
                    var lng = centerElement.GetProperty("lng").GetDouble();
                    var lat = centerElement.GetProperty("lat").GetDouble();
                    Console.WriteLine($"  地图中心: 经度={lng}, 纬度={lat}");
                }
                
                if (mapData.TryGetProperty("zoom", out var zoomElement))
                {
                    var zoom = zoomElement.GetInt32();
                    Console.WriteLine($"  缩放级别: {zoom}");
                }
                
                // 提取overlays数据
                if (mapData.TryGetProperty("overlays", out var overlaysElement))
                {
                    Console.WriteLine($"  Overlays数组类型: {overlaysElement.ValueKind}");
                    
                    var convertedOverlays = ConvertNewFormatOverlaysToMapDrawingData(overlaysElement);
                    
                    Console.WriteLine($"  转换结果: 共 {convertedOverlays.Count} 个图形");
                    
                    foreach (var overlay in convertedOverlays)
                    {
                        Console.WriteLine($"    - 类型: {overlay.ShapeType}, 标签: {overlay.Label}");
                        Console.WriteLine($"      坐标数量: {overlay.Coordinates?.Count ?? 0}");
                        ...
                        Console.WriteLine($"      样式: 描边颜色={overlay.Style?.StrokeColor}, 填充颜色={overlay.Style?.FillColor}");
                    }
                }
                
                Console.WriteLine("  ✓ 新格式数据解析成功\n");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"  ✗ 新格式数据解析失败: {ex.Message}\n");
        ...
        // 新格式数据转换方法
        static List<MapDrawingDto> ConvertNewFormatOverlaysToMapDrawingData(JsonElement overlaysElement)
        {
            var result = new List<MapDrawingDto>();
            ...
            try
            {
                ...
                Console.WriteLine($"转换新格式overlays数据完成，共 {result.Count} 个图形");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"转换新格式overlays数据失败: {ex.Message}");
            }
